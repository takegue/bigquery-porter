create table if not exists examples.sample_sharding_20220101
as
...
FOR record in (
  ...
  execute immediate format("""
    create table if not exists examples.sample_sharding_%s
    as
...
select * from `examples.sample_sharding_*`;
