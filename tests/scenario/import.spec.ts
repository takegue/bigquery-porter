...
describe('CLI: import', () => {
  ...
  it<CLITestContext>(
    `import _testImport bqutil.fn.sure_nonnull`,
    async ({ meta, cli, err, rootPath }) => {
      const dataset = new BigQuery().dataset('_testImport');
      try {
      ...
      expect(files.keys()).toMatchSnapshot('Imported Files: List');
      expect(files).toMatchSnapshot('Imported Files: Contents');
      expect(err).toMatchSnapshot();
    },
  );
});
