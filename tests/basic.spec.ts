...
describe('bigquery: path2bq', () => {
  const cases: Array<{
    ...
    expected: string;
      ...
      {
        input: [
          'bigquery-porter/bigquery/@default/v0/@models/some_model/ddl.sql',
          'bigquery-porter/bigquery',
          'my-project',
        ],
        expected: 'my-project.v0.some_model',
      },
      {
        input: [
...
describe('biquery: getFullResourceId', () => {
  it('Model', async () => {
    const bqObj = new BigQuery()
      .dataset('example_schema', { projectId: 'example_project' })
      .model('model_id');
    expect(getFullResourceId(bqObj)).toBe(
      'example_project:example_schema.model_id',
    );
  });
  ...
  it('Routine', async () => {
    ...
    expect(getFullResourceId(bqObj)).toBe(
      'example_project:example_schema.routine_id',
    );
  });
  ...
  it('Table', async () => {
    ...
    expect(getFullResourceId(bqObj)).toBe(
      'example_project:example_schema.table_id',
    );
  });
  ...
  it('Dataset', async () => {
    ...
    expect(getFullResourceId(bqObj)).toBe(
      'example_project:example_schema',
    );
  });
});

describe('util: humanFileSize', () => {
  const cases: Array<{
  ...
  it.concurrent.each(cases)('normalized bigquery path', async (args) => {
    const { input, expected } = args;
    expect(humanFileSize(...input)).toMatchObject(expected);
  });
});

describe('util: msToTime', () => {
  const cases: Array<{
    input: [number];
    expected: string;
  }> = [
  ...
  it.concurrent.each(cases)('normalized bigquery path', async (args) => {
    const { input, expected } = args;
    expect(msToTime(...input)).toMatchObject(expected);
  });
});
